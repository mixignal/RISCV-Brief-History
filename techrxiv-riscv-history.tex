...
%Saroj, I had a look at the RISC-V article today. It looks good, both content wise and presentation wise.
%There are a few (really only a few) places where small grammatical corrections can be done. The rest is fine. RISC-V has also come up with a 16-bit ISA for reducing code size (and increasing code density) for cost and power sensitive embedded/IoT applications.
%In August 2023 the first RISC-V PC "Roma laptop" featuring quad- core 1.5 GHz 64- bit CPU and a dual core GPU has been released to developers by DeepComputing and Xcalibyte, a Chinese company.
%May be, you can look up and incorporate in the article.
% Another development re. SiFive and Andes
